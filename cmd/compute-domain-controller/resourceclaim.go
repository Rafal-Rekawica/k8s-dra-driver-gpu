			...
			if cd.Status.Nodes != nil {
				for _, node := range cd.Status.Nodes {
					if node.Name == nodeName {
						...
						if node.Status == nvapi.ComputeDomainStatusReady {
							return true, nil
						} else if node.Status == nvapi.ComputeDomainStatusNotReady {
	...
	if hasRef {
		rcMonitors.Range(func(k, v any) bool {
			...
			if ok && monitorInfo != nil {
				monitorInfo.cancel()
				<-monitorInfo.done // Wait for the specific goroutine to complete
			}
